...
class GridWorld:
    def __init__(self, filename, reward=None):
        if reward is None:
            reward = {0: 0, 1: 1.0, 2: -1.0, 3: np.NaN}
        file = open(filename)
        self.map = np.array(
    ...
    def execute_policy(self, policy, start_pos=(2, 0), max_steps=2000):
      s = self.get_state_from_pos(start_pos)
      total_reward = 0
      state_history = [s]
      steps = 0  # Contador de pasos

      while steps < max_steps:  # Límite de pasos
...
reward = ex.execute_policy(policy, start_pos=(0, 0))
