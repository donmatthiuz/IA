    ...
    for iterations in range(maxNumberOfIterations):
        ...
        for state in range(env.observation_space.n):
            ...
            for action in range(env.action_space.n):
                innerSum = 0
                # Aquí usamos `step` para simular las transiciones y calcular la expectativa
                for probability in range(1):  # Para entornos deterministas, la probabilidad siempre es 1
                    nextState, reward, isTerminalState, _, _ = env.step(action)  # Simula un paso
                    innerSum += policy[state, action] * (reward + discountRate * valueFunctionVector[nextState] * (not isTerminalState))
                outerSum += innerSum
...
def evaluate_policy_success_rate(env, policy, num_episodes=1000):
    success_count = 0
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            # Seleccionar la mejor acción de la política
            action = np.argmax(policy[state])
            next_state, reward, done, _, _ = env.step(action)
            state = next_state
            
            # Verificar si el agente alcanzó el objetivo
            if done and reward == 1:  # Recompensa 1 significa que alcanzó el objetivo
                success_count += 1
                break
